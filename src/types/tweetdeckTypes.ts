...
type TweetDeckControllerResolve<T> = (value: T) => void
type TweetDeckControllerReject<T> = (value: T) => void
...
export interface TweetDeckControllerClient {
  ...
  showUser: (
    ...
    resolve: TweetDeckControllerResolve<User>,
    reject: TweetDeckControllerReject<unknown>
  ) => void
  getUsersByIds: (
    user_ids: string[],
    resolve: TweetDeckControllerResolve<User[]>,
  ...
  show: (
    id: string,
    resolve: TweetDeckControllerResolve<unknown>,
    reject: TweetDeckControllerReject<unknown>
  ) => void
  update: (
    status: string,
...
interface TweetDeckController {
  ...
  clients: {
    ...
    // insert
    getPreferredClient: (key: 'twitter') => TweetDeckControllerClient
  };
  scheduler: Scheduler;
